...
window.claimDay = (day) => claimDay(day);
...
function updateCheckInOverlay() {
    const checkInDay = currentUserData.checkInDay || 0;
    const lastCheckIn = currentUserData.lastCheckIn;
    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    ...
    weekDays.forEach((dayEl, index) => {
        ...
        dayEl.classList.remove('claimed', 'current', 'future');
        
        if (dayNum < currentDayOfWeek || (dayNum === 7 && currentDayOfWeek !== 7)) {
            if (dayNum <= checkInDay) {
                dayEl.classList.add('claimed');
            }
        } else if (dayNum === currentDayOfWeek || (currentDayOfWeek === 7 && dayNum === 7)) {
    ...
    if (streakEl) streakEl.textContent = checkInDay;
    
    const progressFill = document.getElementById('checkin-progress-fill');
    if (progressFill) {
        progressFill.style.width = `${(checkInDay / 7) * 100}%`;
    }
    ...
    if (lastCheckIn) {
        ...
        if (lastCheckInDate.getTime() === today.getTime()) {
            ...
            checkInBtn.innerHTML = '<i class="fas fa-check"></i> Claimed Today';
            return;
        }
    }
    checkInBtn.disabled = false;
    checkInBtn.innerHTML = '<i class="fas fa-gift"></i> Claim Today\'s Reward';
}
...
async function claimDay(dayNum) {
    ...
    if (dayNum > currentDayOfWeek && !(currentDayOfWeek === 7 && dayNum === 7)) {
        showToast('Cannot claim future days!', 'error');
        return;
    }
