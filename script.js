...
async function spinWheel() {
    ...
    // Check if we need to reset spins for a new day
    const lastSpin = currentUserData.last_spin;
    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime();
    
    let spinsLeft = currentUserData.spins_left;
    ...
    if (lastSpin) {
        ...
        if (lastSpinDay < today) {
            ...
            currentUserData.spins_left = 7;
            currentUserData.last_spin = now.toISOString();
        }
    } else if (spinsLeft === undefined || spinsLeft === null) {
        spinsLeft = 7;
    }
...
function updateSpinUI() {
    ...
    let spins = currentUserData.spins_left;
    ...
    if (lastSpin) {
        ...
        // If it's a new day, reset spins
        if (lastSpinDay < today) {
            spins = 7; // Reset to 7 spins for new day
            // Update in database
    ...
    } else if (spins === undefined || spins === null) {
        spins = 7; // First time user
    }
    ...
    supabaseClient.from('tasks').select('*').eq('status', 'active').order('id', { ascending: false })
        .then(({ data, error }) => {
            ...
            if (currentTaskFilter === 'all' || currentTaskFilter === 'ad_watch') {
                ...
                const watchedCount = watchedAdIds.length;
                ...
                dailyAds.forEach(ad => {
                    ...
                    div.innerHTML = `
                        <div class="task-header">
                            <span class="task-type">${isWatched ? '✅ Watched' : '📺 Ad Watch'}</span>
                            <span class="task-reward">${ad.reward} HZN</span>
                        </div>
                        <div class="task-title">${ad.title}</div>
                        <div class="task-info">
                            <small>⏱️ ${ad.duration}s</small>
                        </div>
                        ${isWatched 
                            ? '<button class="task-btn completed">✅ Completed</button>' 
                            : `<button class="task-btn" onclick="startAdTask('${ad.id}', ${ad.duration}, ${ad.reward})">Watch</button>`
                        }
                    `;
            ...
            if (currentTaskFilter !== 'ad_watch') {
                const filteredTasks = currentTaskFilter === 'all' 
                    ? tasks 
                    : tasks.filter(t => t.category === currentTaskFilter);
                ...
                filteredTasks.forEach(t => {
                    ...
                    div.innerHTML = `
                        <div class="task-header">
                            <span class="task-type">${getCategoryIcon(t.category)}</span>
                            <span class="task-reward">${t.reward} HZN</span>
                        </div>
                        <div class="task-title">${t.title}</div>
                        <div class="task-info">
                            <small>⏱️ ${t.duration}s</small>
                            ${t.description ? `<small> | ${t.description}</small>` : ''}
                        </div>
                        ${t.link ? `<a href="${t.link}" target="_blank" class="task-link">🔗 Open Task</a>` : ''}
...
async function completeBackendTask(task) {
    ...
    try {
        ...
        if (!error) {
            ...
            showToast(`Task complete! +${task.reward} HZN`);
            loadTasks();
        } else {
...
function shareOnWhatsApp() { 
    ...
    const message = `🎉 Welcome to Millionar Earning App! 💸

Ab hasil karein Rs 4000 tak earning asaani ke sath 🚀

Signup bonus ke liye referral code use karein:
👉 ${code}

Download Now:
https://zfankaar.github.io/Klyneo/

⚡ Limited time opportunity — abhi join karein!`;
...
Ab hasil karein *Rs 4000 tak daily earning* — directly from your phone! 🚀
...
// Save activity - logs for now since we use backend announcements
function saveActivity(title, message) {
    console.log('Activity:', title, '-', message);
    // Could be used to show toast or save to backend later
}
