...
function checkAndResetSpins() {
    ...
    let spins = parseInt(localStorage.getItem('spins_left')) || 7;
    ...
    if (lastSpin) {
        ...
        if (lastSpinDate < today) {
            ...
            localStorage.setItem('spins_left', '7');
            localStorage.setItem('last_spin', newTs);
        }
    } else {
        localStorage.setItem('spins_left', spins.toString());
    }
...
async function spinWheel() {
    ...
    let spinsLeft = parseInt(localStorage.getItem('spins_left')) || 7;
    if (currentUserData?.spins_left !== undefined && currentUserData.spins_left !== null) {
        spinsLeft = currentUserData.spins_left;
    }
        ...
        .then(({ data, error }) => {
            ...
            if (currentTaskFilter !== 'ad_watch') {
                ...
                filteredTasks.forEach(t => {
                    ...
                    div.innerHTML = `
                        <div class="task-header">
                            <span class="task-type">${isCompleted ? '✅ Completed' : getCategoryIcon(t.category)}</span>
                        ...
                        ${!isCompleted && t.link ? `<a href="${t.link}" target="_blank" class="task-link">🔗 Open Task</a>` : ''}
                        <div class="task-btn-container">
                            ${isCompleted 
                                ? '<button class="task-btn completed">✅ Completed</button>' 
